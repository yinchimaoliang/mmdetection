...
def parse_args():
    ...
    parser.add_argument(
        '--result-path', help='path of results.')
    parser.add_argument(
        '--target-path', help='path to save submission')
    parser.add_argument(
...
def main():
    ...
    for root,dirs,files in os.walk(args.result_path):
        for file in files: #遍历每个文件
            data = np.load(os.path.join(args.result_path, file), allow_pickle=True)
            for type in range(types):
                row, col = data[type].shape
                if row != 0 :
                    if float(data[type][0,4]) > args.threshold:
                        jsbase[index[order[type]]].append([file.split('.')[0], float(data[type][0,4]), float(data[type][0,0]),
                         float(data[type][0,1]), float(data[type][0,2]), float(data[type][0,3])])
                    #当这一行存在预测结果时将其记录起来

    with open(os.path.join(args.target_path, 'submission.json'),'w') as f:
